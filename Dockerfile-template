...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && cd openmpi-4.0.0 \
 && ./configure --prefix=/usr/local &> /wrf/wrfoutput/build_log_openmpi_config \
 && make all install &> /wrf/wrfoutput/build_log_openmpi_make \
 && cd / \
...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && ./configure --enable-fortran --enable-cxx --prefix=/usr/local/ &> /wrf/wrfoutput/build_log_hdf5_config \
 && make install &> /wrf/wrfoutput/build_log_hdf5_make \
...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && cd netcdf-c-4.6.2 \
 && ./configure --prefix=${NETCDF} &> /wrf/wrfoutput/build_log_ncc_config \
 && make install &> /wrf/wrfoutput/build_log_ncc_make
RUN source /opt/rh/devtoolset-8/enable \
 && env \
 && cd ${NETCDF}/BUILD_DIR \
 ...
 && CPPFLAGS=-I${NETCDF}/include LDFLAGS=-L${NETCDF}/lib ./configure --prefix=${NETCDF} &> /wrf/wrfoutput/build_log_ncf_config \
 && make install &> /wrf/wrfoutput/build_log_ncf_make
